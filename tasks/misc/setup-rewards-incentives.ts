...
task(
  ...
  const rewardsVault =
    INCENTIVES_REWARDS_VAULT[network] ?? incentivesRewardsVault;
  ...
  const transferStrategyArtifact = await hre.deployments.deploy(
    INCENTIVES_PULL_REWARDS_STRATEGY_ID,
    {
      ...
      from: deployer,
      args: [rewardsProxyAddress, admin, rewardsVault],
  ...
  for (let incentive of incentivesInput) {
    ...
    if (isTestnetMarket(config)) {
      ...
      await waitForTx(
        await rewardToken
          .connect(await hre.ethers.getSigner(incentivesRewardsVault))
          .approve(
            transferStrategyArtifact.address,
    ...
    if (emissionAdmin !== incentivesEmissionManager && emissionAdmin) {
      console.log(
  ...
  if (incentiveConfigList.length) {
    ...
    console.log(
      "- Successfully setup incentives, make sure to perform an ERC20 approve from the rewardsVault [",
      incentivesRewardsVault,
      "] to the PullRewardsTransferStrategy [",
      transferStrategyArtifact.address,
      "]"
    );
  } else {
