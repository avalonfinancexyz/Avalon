...
export const initReservesByHelper = async (
  ...
  let admin = await addressProvider.owner();
  ...
  for (let i = 0; i < reserveSymbols.length; i++) {
    ...
    initInputParams.push({
      ...
      aTokenName: `Avalon ${aTokenNamePrefix} ${reserveSymbols[i]}`,
      aTokenSymbol: `a${symbolPrefix}${reserveSymbols[i]}`,
      variableDebtTokenName: `Avalon ${variableDebtTokenNamePrefix} Variable Debt ${reserveSymbols[i]}`,
      variableDebtTokenSymbol: `variableDebt${symbolPrefix}${reserveSymbols[i]}`,
      stableDebtTokenName: `Avalon ${stableDebtTokenNamePrefix} Stable Debt ${reserveSymbols[i]}`,
...
export const configureReservesByHelper = async (
  ...
  if (tokens.length) {
    ...
    if (isAdmin) {
      ...
      console.log(` - Not pool admin, executed removeRiskAdmin from multisig:`);
      console.log(` - aclManager ${aclManager.address}`);
